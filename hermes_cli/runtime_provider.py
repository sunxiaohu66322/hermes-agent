...
def resolve_runtime_provider(
    *,
    ...
    # Luban (Claude Code CLI) — external subprocess via stream-json protocol.
    # Resolves the `claude` CLI binary and builds the stream-json args.
    if provider == "luban":
    ...
    # Mogong (Codex CLI) — external subprocess via exec --json protocol.
    # Symmetric to luban: resolves the `codex` CLI binary and builds exec args.
    if provider == "mogong":

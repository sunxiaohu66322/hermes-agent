...
class CodexStreamJsonClient:
    ...
    def __init__(
        ...
        **_: Any,
        ...
        # Persistent process attributes (shared across instances via module globals)
    ...
    def close(self) -> None:
        ...
        # Do NOT close _persistent_proc — stays alive for reuse
        # self.is_closed = True  # keep instance usable
        if proc is None:
            return
        try:
            proc.terminate()
    ...
    def _ensure_persistent_process(self):
        """Ensure a shared persistent codex process is alive; recycle if idle."""
        ...
        with _shared_persistent_lock:
            if _shared_persistent_proc and _shared_persistent_proc.poll() is None:
                if time.monotonic() - _shared_last_active > self._idle_timeout:
                    logger.debug("codex shared proc idle %.0fs, recycling",
                    ...
                    return _shared_persistent_proc
            ...
            logger.info("codex shared persistent proc started (pid=%s)", proc.pid)
            return proc

    def _persistent_send_and_recv(self, proc, prompt_text, timeout_seconds):
        """Send a prompt to the persistent codex process and read the result."""
        ...
        if result_text is not None:
            logger.info("codex shared call #%d completed (pid=%s)",
        ...
        stderr_excerpt = "\n".join(stderr_tail[-15:])
        raise RuntimeError(
            f"codex CLI persistent did not produce agent_message. "
...
_singleton_client: Optional["CodexStreamJsonClient"] = None
_singleton_lock = threading.Lock()


def get_shared_client(**kwargs) -> "CodexStreamJsonClient":
    """Get or create a shared CodexStreamJsonClient singleton.

    All delegate_task calls reuse the same codex process, avoiding 40-50s
    cold starts on every call. The persistent process auto-recycles after
    _idle_timeout (default 600s).
    """
    global _singleton_client
    with _singleton_lock:
        if _singleton_client is None:
            _singleton_client = CodexStreamJsonClient(**kwargs)
            logger.info("Created shared CodexStreamJsonClient singleton")

...
def init_agent(
    ...
    if (
        ...
        and agent.provider != "luban"
        and agent.provider != "mogong"
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if api_key and base_url:
            ...
            # copilot-acp, luban (Claude Code CLI) AND mogong (Codex CLI) are
            # external-process transports that carry command/args in
            # agent.acp_command/args. All routed to their client class in
            # agent_runtime_helpers (build_agent_client).
            if agent.provider in ("copilot-acp", "luban", "mogong"):
